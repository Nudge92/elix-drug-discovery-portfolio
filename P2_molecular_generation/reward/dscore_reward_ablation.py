...
class Dscore_reward_ablation(Reward):
    ...
    def calc_reward_from_objective_values(values, conf):
        ...
        objectives = [f.__name__ for f in Dscore_reward_ablation.get_objective_functions(conf)]
        ...
        # === Ablation: aggregation method selection ===
        aggregation = conf.get("aggregation_method", "geometric")

        if aggregation == "geometric":
            # Original Dscore: weighted geometric mean
            multiplication_value = 1
            for v, w in zip(scaled_values, weights):
                multiplication_value *= v**w
            score = multiplication_value ** (1 / sum(weights))

        elif aggregation == "arithmetic":
            # Weighted arithmetic mean
            score = sum(v * w for v, w in zip(scaled_values, weights)) / sum(weights)

        elif aggregation == "tchebycheff":
            # Weighted Tchebycheff: score determined by worst objective
            weighted_values = [v * (w / max(weights)) for v, w in zip(scaled_values, weights)]
            score = min(weighted_values)
